 ...
 * </pre>
 *
 * 提供了两个常用的 Test 接口实现，分别是 IsA 和 AnnotatedWith
 * @author Tim Fennell
 */
public class ResolverUtil<T> {
    ...
    /**
     * Test实现类，用于检测指定类是否继承了 parent 指定的类
    ...
    /**
     * Test实现类 检测指定类是否添加 了 annotation i注解
    ...
    /**
     * 记录了当前使用的类加载器，默认情况下，使用的是当前线程上下文绑定的 ClassLoader，
     * 可以通过 setC!assLoader（）方法修改使用类加载器。
     *
    ...
    public ClassLoader getClassLoader() {
        // 默认情况下，使用的是当前线程上下文绑定的 ClassLoader
     ...
     * 对类型进行匹配，若符合匹配规则，则将其放入内部集合中：org.apache.ibatis.io.ResolverUtil#matches集合中
     */
    public ResolverUtil<T> find(Test test, String packageName) {
        // 根据包名获取其对应的路径 将点替换为/ 如com.yhw.blog
