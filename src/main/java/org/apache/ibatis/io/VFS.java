 ...
 * 单例模式
 * VFS:虚拟文件系统
 * VFS 是一个抽象类 ， MyBatis 中提供了 JBoss6VFS 和 DefaultVFS 两个 VFS 的实现
 * 用户也可以提供 自定义的 VFS 实现类
...
public abstract class VFS {
    ...
    // 记录了用户自定义的 VFS 实现类 。 VFS.addimplClass（）法会将指定的 VFS 实现对应的 Class 对象添加到USER_IMPLEMENTATIONS集合中
    public static final List<Class<? extends VFS>> USER_IMPLEMENTATIONS = new ArrayList<Class<? extends VFS>>();

    /** Singleton instance. */
    // 羊例模式， 记录了全局唯一的 VFS 对象
    ...
    public static VFS getInstance() {
        ...
        // 优先使用用户自定义的VFS实现，如果没有自定义 VFS 实现，则使用 MyBatis 提供的 VFS 实现
